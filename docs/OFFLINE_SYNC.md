...
## PHASE 5H Repair-7 — Runtime identity, cursor and conflict hardening

Database v5 keeps the existing global sync queue and generic sync metadata schema. Because those rows do not contain a workspace/account column, Android now binds each local Room database to exactly one backend account + workspace in the existing encrypted `app_settings` table (`sync.local.identity.binding.v1`). The first authenticated sync/session establishes the binding; later account/workspace changes fail closed before queue claiming or pull application. Existing cursor/server-marker history is inspected before first binding so an upgraded database cannot silently rebind prior sync history to a different workspace. No Room schema or migration is changed.

Pull cursors remain durable per workspace and are committed in the same Room transaction as accepted remote changes. A deterministic local conflict keeps the trusted local value, marks its unresolved queue mutation for manual review when present, records a workspace/entity conflict marker, and allows the cursor to advance past that already-accounted-for remote mutation. Malformed/poisoned remote data does not advance the cursor; instead Android stores a durable workspace pull-block marker so periodic workers do not replay the same poisoned cursor indefinitely. A successful explicit backend re-authentication is the controlled retry signal: it clears the pull block and immediately kicks the unique sync worker. If the remote poison remains, the next pull blocks again without losing the cursor or queue.

Session expiry/revocation clears only the active session token. Local attendance and queued mutations remain available/durable. Re-authentication must resolve back to the bound account/workspace; renewal never silently selects another membership for a bound database. The HTTP client rethrows coroutine cancellation instead of converting cancellation to a retryable network failure.
