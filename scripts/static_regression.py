...
# PHASE 5H Repair-7 sync runtime hardening regression guards.
sync_identity=read(SRC/'com/hammam/attendai/sync/LocalSyncIdentity.kt')
backend_session=read(SRC/'com/hammam/attendai/sync/BackendAccountSessionManager.kt')
http_backend=read(SRC/'com/hammam/attendai/sync/HttpBackendClient.kt')
main_vm=read(SRC/'com/hammam/attendai/ui/MainViewModel.kt')
app_container=read(SRC/'com/hammam/attendai/AppContainer.kt')
sync_rules=read(SRC/'com/hammam/attendai/sync/SyncIntegrityRules.kt')
entities_sync=read(SRC/'com/hammam/attendai/data/local/entity/Entities.kt')
ok('sync.local.identity.binding.v1' in sync_identity and 'AppSettingEntity' in sync_identity,'Repair-7 durable local sync identity binding missing')
ok('bindOrValidate(account,workspace)' in processors,'Repair-7 sync processor does not validate local account/workspace before queue processing')
ok('workspace!!' not in processors,'Repair-7 workspace force unwrap reintroduced')
ok('sync.pull.blocked.' in processors and 'pullBlockRequired' in processors,'Repair-7 poisoned-pull durable block missing')
ok('sync.conflict.$workspace' in processors and 'pullConflictCanAdvance' in processors,'Repair-7 deterministic pull conflict recording missing')
ok('selectWorkspaceForSession' in backend_session and 'memberships.first()' not in backend_session,'Repair-7 session renewal may silently switch workspace')
ok('catch(e:CancellationException){throw e}' in http_backend,'Repair-7 backend HTTP cancellation is swallowed')
ok('workspaceChangeError' in main_vm,'Repair-7 workspace-change local binding guard missing')
ok('resumeAfterReauth' in main_vm and 'kickNetworkQueues' in main_vm,'Repair-7 re-auth does not resume queued sync promptly')
ok('syncIdentityGuard=LocalSyncIdentityGuard' in app_container,'Repair-7 LocalSyncIdentityGuard DI wiring missing')
ok('SYNC_LOCAL_WORKSPACE_BINDING_MISMATCH' in sync_rules and 'SYNC_STORED_MUTATION_INVALID' in sync_rules,'Repair-7 sync integrity codes missing')
# Schema freeze: workspace isolation is implemented via app_settings binding, not Migration 6 / queue schema churn.
sync_queue_match=re.search(r'data class SyncQueueEntity\((.*?)\)',entities_sync,re.S)
ok(sync_queue_match is not None and 'workspace' not in sync_queue_match.group(1),'Repair-7 changed sync_queue schema instead of using v5 identity binding')
