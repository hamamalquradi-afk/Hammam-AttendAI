...
class AppContainer(context:Context){
    ...
    val students=StudentRepository(database,authorization,cipher)
    val dataIntegrity=DataIntegrityRepository(database,authorization)
    ...
    val configurationBackup=com.hammam.attendai.backup.ConfigurationBackupManager(database,preferences,authorization,cipher)
    val reports=ReportRepository(database,authorization)
    val audit=AuditRepository(database,authorization)
    val featureFlags=FeatureFlagRepository(database,authorization)
    ...
    internal val syncIdentityGuard=LocalSyncIdentityGuard(database,cipher::encrypt,cipher::decrypt)
    val backendAccountSessions=BackendAccountSessionManager(preferences,secretStore,syncBackend,syncIdentityGuard)
    val syncProcessor=SyncProcessor(database,syncBackend,cipher::decrypt,cipher::encrypt,{preferences.syncWorkspaceId.first()},{preferences.backendAccountId.first()},{backendAccountSessions.sessionTokenForSync()},{backendAccountSessions.handleAuthFailure(it)},syncIdentityGuard)
    ...
    val aiProviderManager=AiProviderManager(database,cipher,secretStore,backend,{!preferences.backendBaseUrl.first().isNullOrBlank()})
