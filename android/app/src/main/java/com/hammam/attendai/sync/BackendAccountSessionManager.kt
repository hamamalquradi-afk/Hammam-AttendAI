...
class BackendAccountSessionManager(
    ...
    private val identityGuard:LocalSyncIdentityGuard,
){
    ...
    suspend fun provisionCurrentAccount(displayName:String,requestedWorkspaceId:String?):BackendAccountAction{
        identityGuard.provisioningError(requestedWorkspaceId)?.let{return failure(it,localState())}
        ...
        identityGuard.bindOrValidate(accountId,workspaceId)?.let{return identityFailure(it)}
    ...
    suspend fun renewSession():BackendAccountAction{
        ...
        val selection=identityGuard.selectWorkspaceForSession(accountId,currentWorkspace,memberships)
        val selected=selection.workspaceId?:return identityFailure(selection.error?:"MEMBERSHIP_REQUIRED")
        identityGuard.bindOrValidate(accountId,selected)?.let{return identityFailure(it)}
    ...
    private suspend fun identityFailure(error:String):BackendAccountAction{
        secretStore.put(BackendSecretKeys.SESSION_TOKEN,BackendSessionRules.clearedToken())
        preferences.setBackendSession(null,BackendAccountState.UNAUTHENTICATED.name)
        return BackendAccountAction(false,BackendAccountState.UNAUTHENTICATED,error)
    }
