...
fun LocalDataToolsPane(permissions:Set<String>,vm:AdminOperationsViewModel){
    ...
    LaunchedEffect(shareUri){shareUri?.let{uri->try{context.startActivity(Intent.createChooser(Intent(Intent.ACTION_SEND).apply{type="application/octet-stream";putExtra(Intent.EXTRA_STREAM,uri);addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)},context.getString(R.string.share_backup)))}catch(e:android.content.ActivityNotFoundException){android.widget.Toast.makeText(context,"NO_APP_AVAILABLE_FOR_BACKUP_SHARE",android.widget.Toast.LENGTH_LONG).show()}catch(e:SecurityException){android.util.Log.e("LocalDataToolsPane","BACKUP_SHARE_ACCESS_DENIED",e);android.widget.Toast.makeText(context,"BACKUP_SHARE_ACCESS_DENIED",android.widget.Toast.LENGTH_LONG).show()}catch(e:IllegalArgumentException){android.util.Log.e("LocalDataToolsPane","BACKUP_SHARE_URI_INVALID",e);android.widget.Toast.makeText(context,"BACKUP_SHARE_URI_INVALID",android.widget.Toast.LENGTH_LONG).show()}finally{vm.consumeShareBackup()}}}
