...
class ReportProcessor(private val context:Context,private val db:HammamDatabase,private val backend:BackendClient){
        ...
        for(initial in dao.pendingReports(now)){
            ...
            try{
                ...
                if(originalStatus==ReportJobStatus.PENDING_SEND || (job.status==ReportJobStatus.GENERATING && job.generatedAt!=null)){
                    ...
                    if(sha256(file)!=generated.hash)throw PermanentReportFailure("GENERATED_REPORT_HASH_MISMATCH")
                    ...
                    if(r.ok){
                        ...
                        val providerCode=ReportLifecycleRules.safeFailureCode(r.error,if(r.retryable)"REPORT_PROVIDER_TRANSIENT_FAILURE" else "REPORT_PROVIDER_FAILURE")
                        val error=(if(r.retryable)"NETWORK_OR_TRANSIENT:" else "PERMANENT:")+providerCode
            ...
            }catch(e:PermanentReportFailure){
                val code=ReportLifecycleRules.safeFailureCode(e.message,"REPORT_PERMANENT_FAILURE")
                dao.updateReport(job.copy(status=ReportJobStatus.FAILED,errorMessage=code,version=job.version+1))
                dao.insertAudit(AuditLogEntity(UUID.randomUUID().toString(),null,"REPORT_FAILED","ReportJob",job.id,null,null,code,System.currentTimeMillis()))
            }catch(e:CancellationException){
                throw e
            }catch(e:Exception){
                val attempts=job.retryCount+1;val retry=attempts<5;val next=if(!retry)ReportJobStatus.FAILED else if(job.generatedAt==null)ReportJobStatus.SCHEDULED else ReportJobStatus.PENDING_SEND
                val error="TRANSIENT:REPORT_RUNTIME_FAILURE:${e.javaClass.simpleName.take(40)}"
                dao.updateReport(job.copy(status=next,retryCount=attempts,errorMessage=error,version=job.version+1));if(retry)all=false
            }
        };return all
    }
    ...
    private fun generateAttempt(jobId:String,format:String,summary:ReportSummary,records:List<com.hammam.attendai.data.local.dao.ReportAttendanceRow>):Generated{
        ...
        return when(format){
            "CSV"->{val o=csv.generate(attempt,records,statusLabels);Generated(o.file,o.sha256)}
            "SUMMARY"->{
                val text=listOf(
                    context.getString(R.string.app_name),
                    "${context.getString(R.string.total_students)}=${summary.totalStudents}",
                    "${context.getString(R.string.present)}=${summary.present}",
                    "${context.getString(R.string.absent)}=${summary.absent}",
                    "${context.getString(R.string.late)}=${summary.late}",
                    "${context.getString(R.string.partial)}=${summary.partial}",
                    "${context.getString(R.string.left_early)}=${summary.leftEarly}",
                    "${context.getString(R.string.attendance_rate_label)}=${summary.attendanceRate}"
                ).joinToString("\n",postfix="\n")
                attempt.writeText(text,Charsets.UTF_8);Generated(attempt,sha256(attempt))
            }
            else->{
                val labels=ReportPdfLabels(
                    appName=context.getString(R.string.app_name),title=context.getString(R.string.report_file_title),period=context.getString(R.string.report_period_label),
                    totalStudents=context.getString(R.string.total_students),present=context.getString(R.string.present),absent=context.getString(R.string.absent),
                    late=context.getString(R.string.late),partial=context.getString(R.string.partial),leftEarly=context.getString(R.string.left_early),
                    attendanceRate=context.getString(R.string.attendance_rate_label),minuteUnit=context.getString(R.string.minute_short),statusLabels=statusLabels
                )
                val o=pdf.generate(attempt,labels,summary,records);Generated(o.file,o.sha256)
            }
        }
    }
    private fun statusLabels():Map<FinalAttendanceStatus,String> = mapOf(
        FinalAttendanceStatus.PRESENT to context.getString(R.string.present),
        FinalAttendanceStatus.ABSENT to context.getString(R.string.absent),
        FinalAttendanceStatus.LATE to context.getString(R.string.late),
        FinalAttendanceStatus.PARTIAL to context.getString(R.string.partial),
        FinalAttendanceStatus.LEFT_EARLY to context.getString(R.string.left_early),
        FinalAttendanceStatus.EXCUSED to context.getString(R.string.status_excused),
        FinalAttendanceStatus.MANUAL_REVIEW to context.getString(R.string.status_manual_review),
    )
