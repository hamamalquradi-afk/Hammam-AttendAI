...
class SyncProcessor(private val db:HammamDatabase,private val backend:BackendClient,private val decrypt:(String)->String,private val encrypt:(String)->String,private val workspaceProvider:suspend ()->String?,private val accountProvider:suspend ()->String?,private val authTokenProvider:suspend ()->String?,private val onAuthFailure:suspend (String?)->Unit={},private val identityGuard:LocalSyncIdentityGuard){
    ...
    private fun pullBlockKey(workspace:String)="sync.pull.blocked.$workspace"
    ...
    suspend fun process():Boolean{
        ...
        val workspace=workspaceProvider()?.trim()?.takeIf{SyncIntegrityRules.validWorkspaceId(it)}
        if(workspace==null){putSetting("sync.identity.lastError","SYNC_IDENTITY_NOT_CONFIGURED");return true}
        val token=authTokenProvider()
        if(token.isNullOrBlank()){putSetting("sync.identity.lastError","AUTH_REQUIRED");return true}
        val account=accountProvider()?.trim()?.takeIf{it.isNotBlank()}
        val identityError=identityGuard.bindOrValidate(account,workspace)
        if(identityError!=null){putSetting("sync.identity.lastError",identityError);return true}
    ...
    suspend fun resumeAfterReauth(){
        val workspace=workspaceProvider()?.trim()?.takeIf{SyncIntegrityRules.validWorkspaceId(it)}?:return
        putSetting(pullBlockKey(workspace),"")
        putSetting("sync.identity.lastError","")
    }
    ...
    private suspend fun pullAll(workspace:String):Boolean{
        ...
        val pullBlockKey=pullBlockKey(workspace)
        if(!getSetting(pullBlockKey).isNullOrBlank())return true
        ...
        repeat(20){
            ...
            if(!r.ok){
                val code=r.error?:"SYNC_PULL_FAILED"
                recordPullError(pullErrorKey,code,r.responseBody)
                if(SyncIntegrityRules.identityRecoveryRequired(r)){putSetting("sync.identity.lastError",code);onAuthFailure(r.error);return true}
                if(SyncIntegrityRules.pullBlockRequired(code)){recordPullBlock(pullBlockKey,code,r.responseBody);return true}
                return SyncIntegrityRules.permanent(r)
            }
            val root=runCatching{JSONObject(r.responseBody.orEmpty())}.getOrNull()
            if(root==null){recordPullError(pullErrorKey,"SYNC_PULL_PROTOCOL_INVALID",null);recordPullBlock(pullBlockKey,"SYNC_PULL_PROTOCOL_INVALID",null);return true}
            ...
            if(changes==null||hasMore==null||next==null){recordPullError(pullErrorKey,"SYNC_PULL_PROTOCOL_INVALID",r.responseBody);recordPullBlock(pullBlockKey,"SYNC_PULL_PROTOCOL_INVALID",r.responseBody);return true}
            ...
            if(batchError!=null){
                recordPullError(pullErrorKey,batchError,r.responseBody)
                if(SyncIntegrityRules.pullBlockRequired(batchError))recordPullBlock(pullBlockKey,batchError,r.responseBody)
                return true
            }
            try{
                db.withTransaction{
                    for(c in batch){
                        try{
                            ...
                            if(e.entityType!=null&&e.entityId!=null&&SyncIntegrityRules.pullConflictCanAdvance(e.code)){
                                db.coreDao().markEntitySyncConflict(e.entityType,e.entityId,e.code)
                                daoSetting("sync.conflict.$workspace.${e.entityType}.${e.entityId}","${e.code}|serverVersion=${c?.optLong("serverVersion")?:0L}")
                            }else throw e
                        }
                    }
                ...
                if(SyncIntegrityRules.pullBlockRequired(e.code)){recordPullBlock(pullBlockKey,e.code,null);return true}
                return false
            }
            cursor=SyncIntegrityRules.cursorAfterBatch(cursor,next,true)
            if(!hasMore){putSetting(pullErrorKey,"");putSetting(pullBlockKey,"");return true}
        }
    ...
    private suspend fun recordPullBlock(key:String,code:String,responseBody:String?){
        val root=responseBody?.let{runCatching{JSONObject(it)}.getOrNull()}
        val sequence=root?.opt("sequence")?.let{v->if(v is Number)v.toLong().toString() else null}
        val entityId=(root?.opt("entityId") as? String)?.takeIf{it.isNotBlank()}
        putSetting(key,listOfNotNull(code,sequence?.let{"sequence=$it"},entityId?.let{"entityId=$it"}).joinToString("|").take(500))
...
class NotificationProcessor(private val db:HammamDatabase,private val backend:BackendClient,private val decrypt:(String)->String){
    suspend fun process():Boolean{
        ...
        for(candidate in dao.pendingNotifications(System.currentTimeMillis())){
            ...
            val safeError=if(r.ok)null else NotificationDeliveryRules.safeErrorCode(r.error,r.retryable)
            dao.updateNotification(job.copy(status=status,sentAt=if(r.ok)now else null,retryCount=attempts,error=safeError))
            audit(job,if(r.ok)"NOTIFICATION_SEND_ACCEPTED" else if(status==QueueStatus.FAILED)"NOTIFICATION_SEND_RETRYABLE_FAILURE" else "NOTIFICATION_PERMANENT_FAILURE",safeError)
