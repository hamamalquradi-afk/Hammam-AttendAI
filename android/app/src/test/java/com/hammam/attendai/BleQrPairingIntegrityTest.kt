...
class BleQrPairingIntegrityTest {
    ...
    @Test fun usedPairingOfferCannotBeAccepted(){ assertFalse(PairingOfferRules.canAccept("ACCEPTED",2000,1000)) }
    @Test fun rawTokenIsRejectedByQrCodec(){ assertNull(DynamicQrPresencePayload.extractToken("0123456789abcdef")) }
    @Test fun malformedQrPayloadIsRejected(){ assertNull(DynamicQrPresencePayload.extractToken("HAD1|not-a-token")) }
