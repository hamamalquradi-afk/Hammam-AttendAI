...
class SyncFoundationIntegrityTest {
    ...
    @Test fun provisioningAndMembershipFailuresPauseForIdentityRecoveryWithoutInfiniteRetry(){assertTrue(BackendAuthRules.provisioningRequired("PROVISIONING_REQUIRED"));assertTrue(BackendAuthRules.identityRecoveryRequired("PROVISIONING_REQUIRED"));assertTrue(BackendAuthRules.identityRecoveryRequired("MEMBERSHIP_REQUIRED"));assertFalse(SyncIntegrityRules.permanent(BackendResult(false,retryable=false,error="PROVISIONING_REQUIRED")));assertFalse(SyncIntegrityRules.permanent(BackendResult(false,retryable=false,error="MEMBERSHIP_REQUIRED")));assertEquals(com.hammam.attendai.domain.model.QueueStatus.PENDING,SyncIntegrityRules.queueStatusAfterAuthFailure())}
